    ...
    /**
     * Executado ao alterar o valor do campo "Valor do saque" manualmente
     */
    function onKeyUpValorTotal() {
        var oCampoValorTotal = document.querySelector('input[name="valorTotal"]'),
            iValorTotalNumerico = parseFloat(oCampoValorTotal.value.replace("R$", "").trim().replace(".", "").replace(",", "."));
        if (isNaN(iValorTotalNumerico)) { 
            oCampoValorTotal.value = 'R$';
        } else {
            oCampoValorTotal.value = Intl.NumberFormat('pt-BR', {
                                                                    style: 'currency',
                                                                    currency: 'BRL'
                                                                }).format(iValorTotalNumerico);
        }
    }